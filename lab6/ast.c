...
void ASTprint(int level,ASTnode *p)
{
  ...
  case A_VARDEC : 
    PT(level);
    if(p->value > 0) printf("Variable %s %s[%d]\n",ASTtypeToString(
      p->my_data_type),p->name,p->value);
    if(p->value == 0) printf("Variable %s %s\n",ASTtypeToString(
      p->my_data_type),p->name);
    ASTprint(level,p->s1);
  ...
  case A_VAR:
    ...
    else if(p->value == 0) printf("VARIABLE %s\n",p->name);
    break;
...
int check_params(ASTnode *p, ASTnode *q) {
  ...
  while(p != NULL && q != NULL) {
    if(p->type != q->type) {
      printf("Error: parameter %d of function %s is of type %s, but is being passed a %s");
      return 0;
    }
    p = p->next;
    q = q->next;
